...
int executeCommand(char *instruction);
...
int executeLine(char *line)
{
    // Faça uma cópia da linha original
    char lineCopy[1000]; // Substitua MAX_LINE_LENGTH pelo tamanho máximo da linha
    strncpy(lineCopy, line, sizeof(lineCopy));

    // Antes de adicionar à fila, remova qualquer quebra de linha
    size_t length = strlen(lineCopy);
    if (length > 0 && lineCopy[length - 1] == '\n')
    {
        lineCopy[length - 1] = '\0';
    }
    ...
    else if (!strcmp(token, "style sequential"))
    {
    ...
    else if (containsSymbol(token, ">"))
    {
        executeRedirecting(token);
    }
    else if (containsSymbol(token, "|"))
    {
        executePipe(token);
    ...
    else
    {
        ...
        else if (pid1 == 0)
        {
            char *command;
            char *flags;
            divideName(token, &command, &flags);
    ...
    exit (0); // o processo não está sendo encerrado, essa é a causa do loop
    } else {
        waitpid(pid, NULL, 0);
    }
    return 0;
